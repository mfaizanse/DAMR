...
//    Ptr<Params> params = Params::defaultParams();
    ...
    std::cout << "depthscale: " << depth_scale << std::endl;
    std::cout << "w: " << w << "h: " << h << std::endl;

    // Initialize KinFu object
    //kf = KinFu::create(params);
    KinfuPlarr kfx(w, h, depth_scale, 643.338013, 643.096008, 638.95697, 402.330017);
    ...
    while (true)
    {
        ...
        //std::cout << "wwidth: " << width << "h: " << height << std::endl;
...
//        f.release();
        ...
        if (!kfx.update(f))
        {
            kfx.reset(); // If the algorithm failed, reset current state
            ...
            //Affine3f pose = kfx.getPose();
            // std::cout << pose.matrix << std::endl;

            // std::cout << kfx.getPose()<< std::endl;

            kfx.getCurrentPoints();
